...
def log_posterior_probs(x):
    ...
    # logpdf takes the log of the PDF at position x
    log_likelihoods = np.array([dist.logpdf(x).sum() for dist in HYPOTHESIS_SPACE]) 

    log_priors = np.log(PRIOR_PROBS)
    log_evidence = logsumexp(log_likelihoods + log_priors)
    log_p = log_likelihoods + log_priors - log_evidence
